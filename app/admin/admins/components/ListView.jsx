...
export default function ListView() {
  ...
  return (
    <div className="flex-1 flex flex-col gap-4 md:pr-5 md:px-0 px-4">
      <h1 className="text-2xl font-bold text-deepPink-600 mb-4">Admins</h1>
        ...
        <table className="min-w-full border-separate border-spacing-y-3">
          ...
          <tbody>
            {admins?.map((item, index) => (
              <Row key={item?.id || index} item={item} index={index} />
            ))}
          </tbody>
        </table>
      </div>
    ...
    <tr className="bg-white rounded-lg shadow-sm hover:shadow-md transition-all">
      ...
      <td className="border-y px-3 py-2 text-center">
        <div className="flex justify-center">
          <img
            className="h-12 w-12 md:h-14 md:w-14 object-cover rounded-lg border-2 border-lightPink-300 shadow-sm"
      ...
      <td className="border-y px-3 py-2">
        <div className="flex flex-col">
          <h2 className="font-medium text-deepPink-600">{item?.name || "No Name"}</h2>
          <h3 className="text-xs text-gray-500">{item?.email || "No Email"}</h3>
        </div>
      </td>
