...
export default function BrandListView() {
  ...
  return (
    <div className="flex-1 flex flex-col gap-4 md:pr-5 md:px-0 px-4">
      <h1 className="text-2xl font-bold text-deepPink-600 mb-4">Brands</h1>
...
function BrandRow({ item, index }) {
  ...
  return (
    <tr className="bg-white rounded-lg shadow-sm hover:shadow-md transition-all">
      <td className="border-y px-3 py-2 border-l rounded-l-lg text-center">
        {index + 1}
      </td>
      <td className="border-y px-3 py-2 text-center">
        <div className="flex justify-center">
          <img
            className="h-10 w-10 md:h-12 md:w-12 object-cover rounded-lg border-2 border-lightPink-300 shadow-sm"
      ...
      <td className="border-y px-3 py-2">
        <span className="font-medium text-deepPink-600">{item?.name}</span>
      </td>
