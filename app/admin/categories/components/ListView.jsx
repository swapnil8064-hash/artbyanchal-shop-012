...
export default function CategoryListView() {
  ...
  return (
    <div className="flex-1 flex flex-col gap-6 px-4 md:px-6 py-6 md:py-8 bg-gray-50 min-h-screen">
      <h1 className="text-2xl md:text-3xl font-bold text-deepPink-600 text-center mb-6">Categories</h1>

      <div className="overflow-x-auto rounded-xl shadow-lg scrollbar-thin scrollbar-thumb-deepPink-500 scrollbar-track-gray-200">
        <table className="min-w-full bg-white rounded-xl border-separate border-spacing-y-4">
          <thead className="bg-deepPink-100 sticky top-0">
            <tr>
...
function CategoryRow({ item, index }) {
  ...
  return (
    <tr className="bg-white rounded-xl shadow-sm hover:shadow-lg hover:bg-pink-50 transition-all duration-200">
      <td className="px-3 py-2 border-l rounded-l-lg text-center">{index + 1}</td>
      <td className="px-3 py-2 text-center">
        <div className="flex justify-center">
          <img className="h-10 w-10 md:h-12 md:w-12 object-cover rounded-lg border-2 border-deepPink-300 shadow-sm" src={item?.imageURL || "/default-avatar.png"} alt={item?.name} />
        </div>
      </td>
      <td className="px-3 py-2 font-medium text-deepPink-600">{item?.name}</td>
      <td className="px-3 py-2 border-r rounded-r-lg">
        <div className="flex gap-2 justify-center">
          ...
          <button
            ...
            disabled={isDeleting}
            className="bg-gradient-to-r from-deepPink-500 to-pink-600 hover:from-pink-600 hover:to-pink-700 active:scale-95 transition-transform p-2 rounded-lg text-white flex items-center justify-center shadow-md relative"
          >
            {isDeleting ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin mx-auto"></div> : <Trash2 size={16} />}
          </button>
        </div>
      </td>
    </tr>
