...
function CollectionRow({ item, index }) {
  ...
  return (
    <tr className="bg-white rounded-lg shadow-sm hover:shadow-md transition-all">
      ...
      <td className="border-y px-3 py-2 text-center">
        <img className="h-10 w-10 object-cover rounded-lg border-2 border-lightPink-300 shadow-sm mx-auto" src={item?.imageURL || "/default-avatar.png"} alt={item?.title} />
      </td>
      <td className="border-y px-3 py-2 font-medium text-deepPink-600">{item?.title}</td>
      <td className="border-y px-3 py-2">{item?.products?.length}</td>
      <td className="border-y px-3 py-2 border-r rounded-r-lg">
        <div className="flex gap-2 justify-center">
          <button onClick={handleUpdate} disabled={isDeleting} className="bg-lightBlue-300 hover:bg-lightBlue-400 active:scale-95 transition-transform p-2 rounded-lg text-white flex items-center justify-center">
            <Edit2 size={16} />
          </button>
          <button onClick={handleDelete} disabled={isDeleting} className="bg-deepPink-500 hover:bg-salmonPink-500 active:scale-95 transition-transform p-2 rounded-lg text-white flex items-center justify-center relative">
            {isDeleting ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : <Trash2 size={16} />}
          </button>
        </div>
      </td>
    </tr>
